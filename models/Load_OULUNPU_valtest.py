...
class Normaliztion_valtest_image(object):
    """
        same as mxnet, normalize into [-1, 1]
        image = (image - 127.5)/128
    """
    def __call__(self, sample):
        ...
        new_image_x = (image_x - 127.5)/128     # [-1,1]
        return new_image_x
class ToTensor_valtest_img(object):
    """
        Convert ndarrays in sample to Tensors.
        process only one batch every time
    """
    ...
    def __call__(self, sample):
        ...
        return torch.from_numpy(image_x.astype(np.float)).float()
...
class Spoofing_valtest(Dataset):
    ...
    def get_single_image_x(self, image_path, val_map_path, videoname):
        ...
        files_total = len(
            [name for name in os.listdir(image_path) if os.path.isfile(os.path.join(image_path, name))]) // 3
